...
#from scikit-hep import vector

vector.register_awkward()
logging.basicConfig(level=logging.INFO)


def check_assignment_collisions(h1_bs, h2_bs, h3_bs, h1_bb, h2_bb, h3_bb):
    # Convert to numpy for easy manipulation
    h1_bs_flat = ak.to_numpy(ak.flatten(h1_bs))
    h2_bs_flat = ak.to_numpy(ak.flatten(h2_bs))
    h3_bs_flat = ak.to_numpy(ak.flatten(h3_bs))
    h1_bb_flat = ak.to_numpy(ak.flatten(h1_bb))
    h2_bb_flat = ak.to_numpy(ak.flatten(h2_bb))
    h3_bb_flat = ak.to_numpy(ak.flatten(h3_bb))
    
    # Concatenate all assignments
    all_assignments = np.concatenate([h1_bs_flat, h2_bs_flat, h3_bs_flat, h1_bb_flat, h2_bb_flat, h3_bb_flat])

    # Remove invalid (-1) assignments
    valid_assignments = all_assignments[all_assignments != -1]

    # Find duplicates
    unique, counts = np.unique(valid_assignments, return_counts=True)
    duplicates = unique[counts > 1]

    if len(duplicates) > 0:
        print("⚠️ Collision detected! The following jets are assigned multiple times:")
        for jet in duplicates:
            print(f"  - Jet index {jet} appears {counts[np.where(unique == jet)][0]} times")
        return True
    else:
        print("✅ No assignment collisions detected.")
        return False

def check_assignment_collisions_per_event(h1_bs, h2_bs, h3_bs, h1_bb, h2_bb, h3_bb, h1_mask, h2_mask, h3_mask, h1_fj_mask, h2_fj_mask, h3_fj_mask):
    collision_count = 0  # Track how many events have collisions
    
    for i in range(len(h1_bs)):  # Loop over each event
        # Apply masks per event
        h1_bs_valid = h1_bs[i]
        h2_bs_valid = h2_bs[i]
        h3_bs_valid = h3_bs[i]
        h1_bb_valid = h1_bb[i]
        ...
        # Concatenate all valid assignments in this event
        event_assignments = ak.concatenate([h1_bs_valid, h2_bs_valid, h3_bs_valid])
        event_fj_assignments = ak.concatenate([h1_bb,h2_bb,h3_bb])

        # Remove -1 values (invalid assignments)
        event_valid_assignments = event_assignments[event_assignments != -1]

        # Find duplicate assignments within this event
        unique, counts = np.unique(ak.to_numpy(event_valid_assignments), return_counts=True)
        duplicates = unique[counts > 1]

        if len(duplicates) > 0:
            collision_count += 1
            print(f"⚠️ Event {i} has collisions! Duplicated jets: {duplicates}")

    if collision_count == 0:
        print("✅ No assignment collisions detected in any event.")
    else:
        print(f"⚠️ Found {collision_count} events with assignment collisions.")
...
def get_datasets(events):
    ...
    btagpnet = get_n_features("jet{i}PNetB", events, range(1, N_JETS + 1))
    btagpnetcat = get_n_features("jet{i}PNetTagCat", events, range(1, N_JETS + 1))
    ...
    fj_xbbtagcat = get_n_features("fatJet{i}PNetXbbTagCat", events, range(1, N_FJETS + 1))
    ...
    phi = phi[mask]
    btag = btag[mask]
    btagpnet = btagpnet[mask]
    btagpnetcat = btagpnetcat[mask]
    jet_id = jet_id[mask]
    ...
    fj_xbb = fj_xbb[mask]
    fj_xbbtagcat = fj_xbbtagcat[mask]
    fj_xqq = fj_xqq[mask]
    fj_qcd = fj_qcd[mask]
    ...
    indexcat = events.IndexMaxCat[mask]
    #categorisation= events.categorisation[mask]
    #categorisation = ak.Array([int(c) for c in categorisation])
    #print(categorisation)
    #print(type(categorisation))
    

    # mask to define zero-padded small-radius jets
    mask_jet = (pt > MIN_JET_PT)
    ...
    higgs_idx = ak.where(higgs_idx != 0, ak.where(hadron_flavor > -1, higgs_idx, -1), 0)
    ...
    #collision_found = check_assignment_collisions(h1_bs, h2_bs, h3_bs, h1_bb, h2_bb, h3_bb)
    ...
    #for i in range(len(h1_bs)):
    #    print(h1_bs[i], h2_bs[i], h3_bs[i])
    #    print(h1_mask[i], h2_mask[i], h3_mask[i])
    ...
    #collision_found = check_assignment_collisions_per_event(h1_bs, h2_bs, h3_bs, h1_bb, h2_bb, h3_bb, h1_mask, h2_mask, h3_mask, h1_fj_mask, h2_fj_mask, h3_fj_mask)

    datasets = {}


    datasets["INPUTS/Jets/MASK"] = mask_jet.to_numpy()
    ...
    datasets["INPUTS/Jets/btagpnet"] = btagpnet.to_numpy()
    datasets["INPUTS/Jets/btagpnetcat"] = btagpnetcat.to_numpy()
    ...
    #datasets["INPUTS/BoostedJets/MASK"] = mask.to_numpy()
    ...
    datasets["INPUTS/BoostedJets/fj_xbbtagcat"] = fj_xbbtagcat.to_numpy()
    ...
    #datasets["INPUTS/Leptons/MASK"] = mask.to_numpy()
    ...
    datasets["INPUTS/MET/MASK"] = met_mask.to_numpy()
    datasets["INPUTS/MET/met"] = met.to_numpy()
    datasets["INPUTS/INDEX/indexcat"] = indexcat.to_numpy()
    datasets["INPUTS/HT/MASK"] = ht_mask.to_numpy()
    datasets["INPUTS/HT/ht"] = ht.to_numpy()


    # Taus
    datasets["INPUTS/Taus/MASK"] = tau_mask.to_numpy()
    #datasets["INPUTS/Taus/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet1/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet2/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet3/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet4/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet5/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet6/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet7/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet8/MASK"] = mask.to_numpy()
    ...
    #datasets[f"INPUTS/Jet9/MASK"] = mask.to_numpy()
    ...
    #datasets["CLASSIFICATIONS/EVENT/categorisation"] = categorisation.to_numpy()
...
@click.option("--train-frac", default=0.35, help="Fraction for training.")
def main(in_files, out_file, train_frac):
    ...
    for file_name in in_files:
        ...
        skip = True
        for key in mappings:
            if key in file_name: skip = False
        ...
        if 'JetMET' in file_name: continue
        if 'c3' in file_name: continue
        if 'HEFT' in file_name: continue
        if 'Graviton' in file_name: continue
        if 'JES' in file_name: continue
        if 'JER' in file_name: continue
        if 'JMR' in file_name: continue
        if 'NMSSM' in file_name: continue
        if 'powheg2018' in file_name: continue
        if 'ttHToNonbb' in file_name: continue
        if 'ttHToTauTau' in file_name: continue
        if 'QCD_datadriven' in file_name: continue
        if 'DYJetsToLL' in file_name: continue
        ...
        with uproot.open(file_name) as in_file:
            ...
            if "training" in out_file:
                ...
                if "GluGluToHHHTo6B_SM.root" in file_name or "QCD.root" in file_name or "GluGluToHHTo4B_cHHH1.root" in file_name: 
                    entry_stop = int(0.95 * num_entries)
            else:
